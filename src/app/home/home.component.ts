...
export class HomeComponent {
	...
	private _initObservables(): void {
		...
		this.timeText$ = this._changeTracker$.pipe(
			...
			withLatestFrom(this.shouldShowDataMissingBand$),
			map(
				([[[powerData, mostRecentChange], isDataMissing], shouldShowDataMissing]: [
					[[IPowerData[], IRecentSwitch], boolean],
					boolean,
				]): string => {
					if (isDataMissing) {
						return this._getTimeIntervalDisplay(
							shouldShowDataMissing ? this._timeWhenDataStartedMissing$.getValue() : powerData[powerData.length - 2].time,
